  ...
  return (
    <section className="mx-auto max-w-7xl px-5 py-10 mt-4 md:mt-0 space-y-6 md:space-y-10">
      <header className="mb-10 flex flex-col md:flex-row items-center justify-between">
        ...
        <Listbox value={selected} onChange={setSelected}>
          <div className="relative w-56">
            <Listbox.Button className="w-full rounded-lg border bg-white py-2 pl-3 pr-10 text-left shadow-sm focus:outline-none focus:ring-brand-500 dark:bg-slate-800">
      ...
      <ul className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3 p-2">
        {filtered.map(a => {
          const cat = categoriesById[a.category?.id];
          return (
            <motion.li
              key={a.id}
            ...
            >
              ...
              <span
                className={`mt-4 inline-block rounded px-2 py-0.5 text-xs font-medium ${
                  cat?.badge ?? 'bg-slate-100 text-slate-600'
                } dark:bg-opacity-20`}
              >
